...
public class BaseInterceptor implements HandlerInterceptor {
    ...
    private Cache cache = Cache.single();
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        LOGGER.info("用户访问地址：{}，来路地址：{}",uri, Kit.getIpAddressByRequest1(request));

        // 请求拦截处理
        UserDomain user = Tale.getLoginUser(request);
        if (null == user) {
            Integer uid = Tale.getCookieUid(request);
            if (null != uid) {
