...
public class Tale {
    ...
    public static Integer getCookieUid(HttpServletRequest request){
        if (null != request) {
            ...
            if (cookie != null && cookie.getValue() != null) {
                try {
                    String uid = fault.deAes(cookie.getValue(), WebConst.AES_SALT);
                    return StringUtils.isNotBlank(uid) && fault.isNumber(uid) ? Integer.valueOf(uid) : null;
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    public static void setCookie(HttpServletResponse response, Integer uid) {
        try {
            String val= fault.enAes(uid.toString(), WebConst.AES_SALT);
            boolean isSSL = false;
